...
namespace nam
{
    ...
    std::unique_ptr<DSP> get_dsp(const unsigned char* modelData, size_t dataSize)
    {
        dspData temp;
        return get_dsp(modelData, dataSize, temp);
    }

    std::unique_ptr<DSP> get_dsp(const unsigned char* modelData, size_t dataSize, dspData& returnedConfig)
    {
        nlohmann::json j = nlohmann::json::parse(modelData, modelData + dataSize);
...
std::vector<float> GetWeights(const float* weightsArray, size_t size)
{
  ...
  // Create a vector from the array of floats
  std::vector<float> weights(weightsArray, weightsArray + size);
...
/* std::unique_ptr<DSP> get_dsp(const unsigned char* modelData, size_t dataSize, dspData& returnedConfig)
{
  ...
  std::vector<float> weights = GetWeights(weightsData, sizeof(weightsData) / sizeof(weightsData[0]));

  // Assign values to returnedConfig
  returnedConfig.version = version;
  returnedConfig.architecture = architecture;
  // returnedConfig.config = nlohmann::json::parse(jsonString); // I copied config manually
  returnedConfig.config = nullptr;
  returnedConfig.metadata = nullptr;
  returnedConfig.weights = weights;
  returnedConfig.expected_sample_rate = -1.0;
...
std::unique_ptr<DSP> get_dsp(dspData& conf)
{
  ...
  else if (architecture == "ConvNet")
  {
    ...
    // std::vector<int> dilations = config["dilations"];
    // const std::string activation = config["activation"];
    // out = std::make_unique<convnet::ConvNet>(channels, dilations, batchnorm, activation, weights,
    // expectedSampleRate);
  }
  else if (architecture == "LSTM")
  {
  ...
  else if (architecture == "WaveNet")
  {
    ...
    //
    layer_array_params.push_back(
      wavenet::LayerArrayParams(1, 1, 8, 16, 3, {1, 2, 4, 8, 16, 32, 64, 128, 256, 512}, "Tanh", false, false));
    layer_array_params.push_back(
      wavenet::LayerArrayParams(16, 1, 1, 8, 3, {1, 2, 4, 8, 16, 32, 64, 128, 256, 512}, "Tanh", false, true));
    const bool with_head = false;
    const float head_scale = 0.02;
    out = std::make_unique<wavenet::WaveNet>(layer_array_params, head_scale, with_head, weights, expectedSampleRate);
  }
  else
  {
  ...
  // "pre-warm" the model to settle initial conditions
  // Can this be removed now that it's part of Reset()?
  out->prewarm(); //TODO: CRASHES WHEN _LAYER_ARRAYS IS 'EMPTY'
